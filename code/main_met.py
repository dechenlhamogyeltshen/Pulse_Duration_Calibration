...
crpath = os.path.join(project_dirs['input'],'met.csv')
...
durations = np.arange(1.0, 20.0, 0.5)  # CME durations in hours
...
for _, onecme in crlist.iterrows():
    ...
    # Spheroidal cone cme
    sph_tt_val = np.nan
    sph_tt_val = spheroidal(onecme, model)
    ...
    for i, duration in enumerate(durations):
        tt_val = np.nan
        tt_val = fixed_duration(onecme, model,duration)
